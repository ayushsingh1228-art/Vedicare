...
export default function Navbar() {
  ...
  return (
    <>
        ...
        <div className="max-w-6xl mx-auto px-5 md:px-8 py-3.5 flex items-center justify-between gap-4">
          ...
          >
            <div className="w-9 h-9 rounded-xl bg-[#C85A17] flex items-center justify-center shadow-md shadow-saffron/20">
              <Leaf className="text-white" style={{ width: "1.1rem", height: "1.1rem" }} />
            </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            {deferredPrompt && (
              <button onClick={handleInstallClick} className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium bg-saffron-light/50 text-saffron border border-saffron/20 hover:bg-saffron hover:text-white transition-all">
            ...
            <button type="button" onClick={toggleLanguage} aria-pressed={isHindi}
              className={`hidden sm:flex items-center gap-1.5 rounded-xl border px-3 py-2 text-xs font-semibold transition-all ${
                isHindi ? "border-saffron bg-saffron-light text-saffron" : "border-[#E8E1D5] bg-white/80 dark:bg-slate-800/90 dark:border-slate-700 text-ink/60 dark:text-slate-300 hover:border-saffron/50 hover:text-saffron"
              }`}
            >
      ...
      {/* EmergencyRideModal is OUTSIDE the header to avoid z-index/backdrop-blur clipping */}
