...
import {
  ...
  AlertTriangle, Stethoscope, PlusCircle, ShieldAlert, Activity,
} from "lucide-react";
...
export default function DoctorDashboard() {
  ...
  return (
    <div className="min-h-screen" style={{ background: "radial-gradient(ellipse at 15% 10%, rgba(200,90,23,0.07) 0%, transparent 45%), radial-gradient(ellipse at 85% 85%, rgba(79,121,66,0.07) 0%, transparent 45%), #FAF9F6" }}>
      <Navbar />
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="text-ink/55 mt-3 text-lg">Review appointments, document conditions, access patient records.</p>
        </motion.div>
        ...
        <div className="flex flex-wrap gap-4 mt-8 mb-10">
          {[
            ...
            { label: "Approved", value: decided.filter(a=>a.status==="approved").length, color: "text-emerald-700", bg: "bg-emerald-50 border-emerald-200" },
            { label: "Patients", value: patients.length, color: "text-blue-700",   bg: "bg-blue-50 border-blue-200" },
          ].map((s) => (
            <div key={s.label} className={`flex items-center gap-3 ${s.bg} border rounded-2xl px-6 py-3`}>
              ...
              <span className="text-sm text-ink/55 font-medium">{s.label}</span>
            </div>
          ))}
        </div>
        ...
        <div className="flex gap-2 mb-7">
            ...
            <button key={t.key} onClick={() => setTab(t.key)}
              className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all ${
                tab === t.key ? "bg-saffron text-white shadow-lg shadow-saffron/25" : "bg-white border border-[#E8E1D5] text-ink/55 hover:border-saffron/40 hover:text-ink"
              }`}>
        ...
        {tab === "appointments" && (
          <div className="space-y-10">
            <section>
              <h2 className="font-serif text-2xl text-ink mb-4">Awaiting approval <span className="text-saffron">({pending.length})</span></h2>
              {pending.length === 0 ? (
                <div className="bg-white border border-[#E8E1D5] rounded-3xl p-12 text-center text-ink/45">
                  <CalendarCheck className="w-10 h-10 mx-auto mb-3 text-herb opacity-50" />
                ...
                <div className="space-y-4">
                  {pending.map((a) => (
                    <motion.div key={a.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
                      ...
                      className="bg-white border border-[#E8E1D5] rounded-3xl p-6 flex flex-wrap items-center justify-between gap-4 hover:shadow-md transition">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-herb-light to-emerald-50 flex items-center justify-center border border-emerald-100 text-lg font-bold text-herb">
                          {a.patient_name.charAt(0)}
                        </div>
                        <div>
                          <div className="font-serif text-2xl text-ink">{a.patient_name}</div>
                          <div className="text-sm text-ink/50 flex items-center gap-1.5 mt-0.5">
                            <Clock className="w-3.5 h-3.5" /> {a.date} · {a.time}
                          </div>
                          {a.reason && <p className="text-sm text-ink/65 mt-2 max-w-md">{a.reason}</p>}
                        </div>
                      </div>
                      <div className="flex gap-2 flex-wrap">
                        <button onClick={() => openPanel({ patient_id: a.patient_id, patient_name: a.patient_name })}
                          className="rounded-xl border border-[#E8E1D5] px-4 py-2.5 text-sm font-medium flex items-center gap-2 hover:border-blue-400 hover:text-blue-600 hover:bg-blue-50 transition">
                        ...
                        <button data-testid={`reject-${a.id}`} onClick={() => act(a.id, "rejected")}
                          className="rounded-xl border border-[#E8E1D5] px-5 py-2.5 font-semibold flex items-center gap-2 hover:border-red-400 hover:text-red-600 hover:bg-red-50 transition">
            ...
            <section>
              <h2 className="font-serif text-2xl text-ink mb-4">Recent decisions</h2>
              {decided.length === 0 ? <p className="text-ink/40">Nothing here yet.</p> : (
                <div className="grid md:grid-cols-2 gap-4">
                  {decided.map((a) => (
                    <div key={a.id} className="bg-white border border-[#E8E1D5] rounded-2xl p-5 flex justify-between items-center hover:shadow-sm transition">
                      <div>
                        <div className="font-serif text-xl text-ink">{a.patient_name}</div>
                        <div className="text-sm text-ink/45 mt-0.5">{a.date} · {a.time}</div>
                      </div>
                      <div className="flex items-center gap-2">
                        <button onClick={() => openPanel({ patient_id: a.patient_id, patient_name: a.patient_name })}
                          className="p-2 rounded-xl hover:bg-blue-50 text-ink/35 hover:text-blue-600 transition" title="Records & Notes">
        ...
        {tab === "patients" && (
          <section>
            <p className="text-ink/45 text-sm mb-6">Patients who have booked with you. Click to view records or add a medical note.</p>
            {patients.length === 0 ? (
              <div className="bg-white border border-[#E8E1D5] rounded-3xl p-14 text-center">
                <User className="w-10 h-10 mx-auto mb-3 text-ink/25" />
                <p className="font-serif text-xl text-ink">No patients yet.</p>
              </div>
            ) : (
              <div className="grid md:grid-cols-2 gap-4">
                {patients.map((p, i) => (
                  <motion.button key={p.patient_id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
                    onClick={() => openPanel(p)}
                    className="text-left bg-white border border-[#E8E1D5] rounded-2xl p-5 flex items-center gap-4 hover:border-saffron/50 hover:shadow-lg hover:-translate-y-0.5 transition-all group">
                    <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-saffron-light to-herb-light flex items-center justify-center text-base font-bold text-saffron border border-[#E8E1D5] shrink-0">
                      {p.patient_name.charAt(0).toUpperCase()}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="font-serif text-xl text-ink truncate">{p.patient_name}</div>
                      <div className="text-xs text-ink/40 mt-0.5">View records · Add medical note</div>
                    </div>
                    <ChevronRight className="w-5 h-5 text-ink/25 group-hover:text-saffron transition-colors shrink-0" />
                  </motion.button>
                ))}
              </div>
            )}
        ...
        {panelOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed top-0 right-0 h-full w-full max-w-lg bg-[#FAF9F6] border-l border-[#E8E1D5] z-50 flex flex-col shadow-2xl overflow-hidden">

              {/* Panel header */}
              <div className="flex items-center gap-3 px-6 py-5 border-b border-[#E8E1D5] bg-white shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-serif text-xl text-ink truncate">{selected?.patient_name}</div>
                  <div className="text-xs text-ink/45">Patient Health File</div>
                </div>
                <button onClick={closePanel}
                  className="w-9 h-9 rounded-xl flex items-center justify-center text-ink/35 hover:text-ink hover:bg-black/5 transition">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              {/* Inner tabs */}
              <div className="flex gap-2 px-5 pt-4 pb-3 shrink-0 border-b border-[#E8E1D5] bg-white">
                {[
                  ...
                  <button key={t.key} onClick={() => setFormTab(t.key)}
                    className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold transition-all ${
                      formTab === t.key ? "bg-saffron text-white shadow-md shadow-saffron/20" : "bg-gray-50 text-ink/55 hover:text-ink border border-[#E8E1D5]"
                    }`}>
              ...
              {formTab === "records" && (
                <div className="flex-1 overflow-y-auto p-5 space-y-3">
                  {recordsLoading ? (
                    <div className="flex flex-col items-center justify-center h-48 gap-3 text-ink/45">
                      <Loader2 className="w-7 h-7 animate-spin text-saffron" />
                      <p className="text-sm">Loading records…</p>
                    </div>
                  ) : records.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-48 text-center gap-3">
                      <FolderOpen className="w-10 h-10 text-ink/20" />
                      <p className="font-serif text-lg text-ink">No records yet</p>
                      <p className="text-xs text-ink/40 max-w-xs">Switch to "Add Medical Note" to create the first entry.</p>
                    </div>
                  ) : (
                    <>
                      <p className="text-xs text-ink/35 font-semibold uppercase tracking-widest pb-1">
                      ...
                      {records.map((rec, i) => {
                        ...
                        return (
                          <motion.div key={rec.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                            className={`bg-white border rounded-2xl p-5 transition hover:shadow-md ${rec.is_serious ? "border-red-300 ring-1 ring-red-200" : "border-[#E8E1D5]"}`}>

                            {/* Serious badge */}
                            {rec.is_serious && (
                              <div className="flex items-center gap-1.5 text-xs font-bold text-red-600 bg-red-50 border border-red-200 rounded-xl px-3 py-1.5 mb-3 w-fit">
                            ...
                            <div className="flex items-start gap-3 mb-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="text-[10px] uppercase tracking-widest text-ink/35 font-bold">{meta.label}</div>
                                <h3 className="font-serif text-lg text-ink leading-tight truncate mt-0.5">{rec.title}</h3>
                            ...
                            {rec.notes && (
                              <p className="text-sm text-ink/65 leading-relaxed bg-[#FAF9F6] rounded-xl p-3 border border-[#E8E1D5] whitespace-pre-wrap">
                                {rec.notes}
                              </p>
                            )}
                            {rec.image_url && (
                              <div className="mt-3 overflow-hidden rounded-xl border border-[#E8E1D5] bg-[#FAF9F6]">
              ...
              {formTab === "add" && (
                <form onSubmit={saveCondition} className="flex-1 overflow-y-auto p-5 space-y-4">
                  ...
                  {/* Diagnosis title */}
                  <div>
                    <label className="text-xs font-semibold text-ink/50 uppercase tracking-wider block mb-1.5">Diagnosis / Title *</label>
                    <input required value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })}
                      placeholder="e.g. Hypertension Stage 1"
                      className="w-full px-4 py-3 rounded-xl border border-[#E8E1D5] bg-white focus:outline-none focus:border-saffron text-sm transition" />
                  </div>

                  {/* Condition */}
                  <div>
                    <label className="text-xs font-semibold text-ink/50 uppercase tracking-wider block mb-1.5">Medical Condition *</label>
                    <textarea required rows={3} value={form.condition} onChange={(e) => setForm({ ...form, condition: e.target.value })}
                      ...
                      className="w-full px-4 py-3 rounded-xl border border-[#E8E1D5] bg-white focus:outline-none focus:border-saffron text-sm transition resize-none" />
                  </div>

                  {/* Severity */}
                  <div>
                    <label className="text-xs font-semibold text-ink/50 uppercase tracking-wider block mb-2">Severity Level</label>
                      ...
                      {Object.entries(SEVERITY).map(([key, val]) => (
                        <button key={key} type="button" onClick={() => setForm({ ...form, severity: key, is_serious: ["severe","critical"].includes(key) })}
                          className={`flex flex-col items-center gap-1.5 py-3 rounded-xl border text-xs font-semibold transition-all ${
                            form.severity === key ? `${val.color} shadow-md scale-105` : "border-[#E8E1D5] bg-white text-ink/50 hover:border-gray-300"
                          }`}>
                  ...
                  {/* Doctor notes */}
                  <div>
                    <label className="text-xs font-semibold text-ink/50 uppercase tracking-wider block mb-1.5">Doctor's Notes (optional)</label>
                  ...
                  {/* Serious flag */}
                  <label className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3 cursor-pointer hover:bg-red-100 transition">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-red-600/70 mt-0.5">Patient will see a red alert on this record</div>
                    </div>
                  </label>
